...
class TestDocumentMetadata:
    ...
    def test_extract_file_metadata_success(self, mock_guess_type, mock_stat, mock_exists):
        """Test successful metadata extraction."""
        mock_exists.return_value = True
        mock_stat.return_value = Mock(st_size=1024, st_ctime=1609459200, st_mtime=1609459300)
        mock_guess_type.return_value = ('application/pdf', None)
        
        with patch('helpers.document_processor.DocumentMetadata._calculate_file_hash', return_value='abcd1234'):
    ...
    def test_extract_content_metadata_with_elements(self):
        ...
        mock_element2.__str__ = Mock(return_value='This is a test narrative with multiple words.')
...
class TestAtlasDocumentProcessor:
    ...
    def test_init_success(self, mock_path_manager, mock_metadata_manager, mock_load_config, 
                         mock_unstructured_available, mock_config):
    ...
    @patch('os.path.exists')
    def test_process_document_file_not_found(self, mock_exists, mock_unstructured_available, mock_config):
    ...
    @patch('os.path.exists')
    def test_process_document_unsupported_format(self, mock_exists, mock_unstructured_available, mock_config):
    ...
    def test_process_document_success(self, mock_content_metadata, mock_file_metadata, 
                                     mock_exists, mock_unstructured_available, mock_config):
        ...
        with patch.object(processor, '_partition_document', return_value=mock_elements):
            with patch.object(processor, '_elements_to_text', return_value='Test content'):
                with patch.object(processor, '_elements_to_structured', return_value=[]):
                    with patch.object(processor, '_save_processed_document'):
                        with patch.object(processor, '_generate_document_uid', return_value='test123'):
    ...
    @patch('json.dump')
    def test_save_processed_document(self, mock_json_dump, mock_file_open, mock_makedirs,
                                    mock_unstructured_available, mock_config):
    ...
    @patch('os.path.exists')
    def test_process_directory_not_found(self, mock_exists, mock_unstructured_available, mock_config):
    ...
    @patch('os.path.exists')
    def test_process_directory_success(self, mock_exists, mock_unstructured_available, mock_config):
        ...
        with patch.object(processor, 'get_supported_extensions', return_value=['.pdf', '.docx']):
            with patch('pathlib.Path.glob') as mock_glob:
                # Mock glob to return different results for each extension
                mock_glob.side_effect = lambda pattern: mock_files if '.pdf' in pattern else []
    ...
    @patch('json.load')
    @patch('builtins.open', new_callable=mock_open)
    def test_get_processing_stats(self, mock_file_open, mock_json_load, mock_glob,
                                 mock_exists, mock_unstructured_available, mock_config):
...
class TestGlobalFunctions:
    ...
    def test_get_document_processor_success(self, mock_processor_class, mock_unstructured_available):
    ...
    def test_process_document_file_success(self, mock_get_processor, mock_unstructured_available):
        """Test convenience function for processing document."""
        mock_processor = Mock()
        mock_processor.process_document.return_value = {'processing_status': 'completed'}
    ...
    @patch('helpers.document_processor.get_document_processor')
    def test_process_document_file_error(self, mock_get_processor, mock_unstructured_available):
...
class TestDocumentProcessorIntegration:
    ...
    def test_process_text_document_integration(self, mock_unstructured_available):
        ...
        try:
            ...
            mock_elements = [Mock()]
            mock_elements[0].text = "This is a test document.\nIt has multiple lines.\nAnd some content."
            ...
            with patch('helpers.document_processor.partition', return_value=mock_elements):
