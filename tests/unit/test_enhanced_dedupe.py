...
class TestEnhancedDeduplicator:
    ...
    def test_fast_duplicate_check(self, deduplicator):
        ...
        with patch.object(deduplicator, 'get_content_hash_index') as mock_index:
            ...
            with patch('helpers.enhanced_dedupe.ContentHasher.title_hash', return_value='abcd1234'):
                metadata = {'title': 'Test Title'}
                assert deduplicator.fast_duplicate_check(ContentType.ARTICLE, metadata) is True
            
            # Test no duplicate
            with patch('helpers.enhanced_dedupe.ContentHasher.title_hash', return_value='different'):
                with patch('helpers.enhanced_dedupe.ContentHasher.content_hash', return_value='different'):
                    with patch('helpers.enhanced_dedupe.ContentHasher.fuzzy_content_hash', return_value='different'):
                        metadata = {'title': 'Different Title', 'content': 'Different content'}
                        assert deduplicator.fast_duplicate_check(ContentType.ARTICLE, metadata) is False
...
class TestConvenienceFunctions:
    ...
    def test_detect_near_duplicates_no_duplicates(self):
        ...
        items = [
            {'uid': 'item1', 'title': 'Machine Learning Algorithms', 'content': 'Deep dive into neural networks and optimization'},
            {'uid': 'item2', 'title': 'Web Development Frameworks', 'content': 'Comparison of React, Angular, and Vue for frontend development'},
        ]
