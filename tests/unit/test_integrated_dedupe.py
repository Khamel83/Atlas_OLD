...
from helpers.integrated_dedupe import (
...
class TestIntegratedDeduplicator:
    ...
    def deduplicator(self, mock_config):
        ...
        with patch('helpers.integrated_dedupe.MetadataManager') as mock_mm, \
             patch('helpers.integrated_dedupe.PathManager') as mock_pm, \
             patch('helpers.integrated_dedupe.create_enhanced_deduplicator') as mock_ed:
    ...
    def test_init_with_defaults(self):
        ...
        with patch('helpers.integrated_dedupe.load_config') as mock_config, \
             patch('helpers.integrated_dedupe.MetadataManager') as mock_mm, \
             patch('helpers.integrated_dedupe.PathManager') as mock_pm, \
             patch('helpers.integrated_dedupe.create_enhanced_deduplicator') as mock_ed:
    ...
    def test_is_content_duplicate_similarity_match(self, deduplicator):
        ...
        deduplicator.enhanced_deduplicator.is_content_duplicate.return_value = (True, similarity_match)
    ...
    def test_check_all_duplicates_content_duplicate(self, deduplicator):
        ...
        with patch.object(deduplicator, 'is_url_duplicate', return_value=False), \
    ...
    def test_check_all_duplicates_no_duplicate(self, deduplicator):
        ...
        with patch.object(deduplicator, 'is_url_duplicate', return_value=False), \
             patch.object(deduplicator, 'is_content_duplicate', return_value=(False, None)):
    ...
    def test_get_duplicate_statistics(self, deduplicator):
        ...
        mock_matches = [
            SimilarityMatch('item1', 'item2', 0.85, 'title_high', 0.9)
        ]
        deduplicator.enhanced_deduplicator.find_duplicates.return_value = mock_matches
    ...
    def test_cleanup_duplicates_dry_run(self, deduplicator):
        ...
        mock_matches = [
            SimilarityMatch('item1', 'item2', 0.98, 'title_exact', 0.99)
        ]
        deduplicator.enhanced_deduplicator.find_duplicates.return_value = mock_matches
        
        result = deduplicator.cleanup_duplicates(dry_run=True, confidence_threshold=0.95)
