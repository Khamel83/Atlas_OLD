#!/usr/bin/env python3
"""Add ALL podcasts from your source of truth to database - no more CSV files"""
...
PODCASTS = [
    ...
    ("Other", "The Town with Matthew Belloni", 0, 1, 1, 0),
    ("Other", "The Urbanist Agenda", 0, 0, 0, 1),
    ("Other", "The Watch", 10, 1, 1, 0),
    ("Other", "The Zach Lowe Show", 0, 1, 1, 0),
...
def main():
    ...
    conn.execute("""
        CREATE TABLE podcasts (
            ...
            name TEXT UNIQUE,
            category TEXT,
            count_target INTEGER DEFAULT 0,
            future_episodes INTEGER DEFAULT 0,
            transcript_only INTEGER DEFAULT 0,
    ...
    for category, name, count, future, transcript_only, exclude in PODCASTS:
        conn.execute("""
            INSERT OR REPLACE INTO podcasts (
                name, category, count_target, future_episodes,
                transcript_only, excluded, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            name, category, count, future, transcript_only, exclude,
