...
def create_htpasswd_file(username, password):
    ...
    run_command("sudo chown www-data:www-data " + htpasswd_path, "Setting htpasswd ownership")
...
def configure_nginx_auth():
    ...
    if not password:
        ...
        print("Please save this password - you'll need it to access your Atlas instance")
    ...
    if "auth_basic" not in config:
        ...
        for line in lines:
            ...
            if line.strip().startswith("server {") and "443 ssl" in config.split(line)[1][:200]:
                in_https_block = True
        ...
        run_command("sudo cp /tmp/atlas_nginx_auth.conf " + nginx_config_path, "Updating nginx configuration")
...
def setup_ip_whitelist():
    ...
    if allowed_ips:
        ...
        if "allow" not in config or "deny all" not in config:
            ...
            run_command("sudo cp /tmp/atlas_nginx_whitelist.conf " + nginx_config_path, "Updating nginx configuration with IP whitelist")
            run_command("sudo nginx -t", "Testing nginx configuration")
            run_command("sudo systemctl reload nginx", "Reloading nginx")
