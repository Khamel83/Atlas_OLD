...
class AtlasPodCLI:
    ...
    def cmd_discover(self, args):
        ...
        for podcast in podcasts:
            ...
            try:
                ...
                for rss_episode in episodes:
                    try:
                        ...
                        episode = Episode(
                            ...
                            url=rss_episode.url,
                            publish_date=rss_episode.publish_date.isoformat() if rss_episode.publish_date else None,
                        ...
                        if rss_episode.transcript_links:
                            ...
                            episode.transcript_url = rss_episode.transcript_links[0]  # Use first link
                            transcripts_found += 1
                        ...
                        logger.error(f"Error processing episode {rss_episode.title}: {e}")
                        errors += 1
                ...
                print(f"   📊 Found {episodes_found} episodes, {transcripts_found} with transcripts")
                ...
                duration = time.time() - start_time
                self.store.complete_discovery_run(
                    run_id, 0, 0, 1, duration, "failed"
                )
                
        print(f"✅ Discovery complete: {total_episodes} episodes, {total_transcripts} transcripts found")
        return True
    ...
    def cmd_fetch_transcripts(self, args):
        ...
        print(f"🎯 Transcript fetching complete: {total_fetched} fetched, {total_failed} failed")
        return True
    
    def _fetch_episode_transcript(self, episode: Episode, podcast: Podcast, 
                                podcast_config: Dict[str, Any]) -> bool:
                    ...
                    if 'content' in source['metadata']:
                        ...
                        if content and len(content) > 100:
                            ...
                            transcript_path = self.exporter.export_transcript(
                                podcast, episode, content, source['url'], source['metadata']
                            )
...
def main():
    ...
    # Global options
    parser.add_argument('--db-path', help='Database path (default: data/podcasts/atlas_podcasts.db)')
    ...
    validate_parser.add_argument('--csv', required=True, help='Path to podcast CSV file')
    
    # register command
    register_parser = subparsers.add_parser('register', help='Register podcasts from CSV')
    register_parser.add_argument('--csv', required=True, help='Path to podcast CSV file')
    
    # discover command
    discover_parser = subparsers.add_parser('discover', help='Discover episodes and transcripts')
    discover_group = discover_parser.add_mutually_exclusive_group(required=True)
    discover_group.add_argument('--all', action='store_true', help='Process all podcasts')
    discover_group.add_argument('--slug', help='Process specific podcast by slug')
    
    # fetch-transcripts command
    fetch_parser = subparsers.add_parser('fetch-transcripts', help='Fetch transcript content')
    fetch_group = fetch_parser.add_mutually_exclusive_group(required=True)
    fetch_group.add_argument('--all', action='store_true', help='Fetch all available transcripts')
    fetch_group.add_argument('--slug', help='Fetch transcripts for specific podcast')
    
    # watch command
    watch_parser = subparsers.add_parser('watch', help='Watch mode for continuous discovery')
    watch_parser.add_argument('--all', action='store_true', help='Watch all podcasts')
    watch_parser.add_argument('--interval', default='30m', help='Discovery interval (default: 30m)')
