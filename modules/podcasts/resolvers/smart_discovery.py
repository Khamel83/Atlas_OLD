...
class SmartDiscoveryResolver:
    ...
    def __init__(self, data_dir: str = "data/podcasts", user_agent: str = "Atlas-Pod/1.0"):
    ...
    def resolve(self, episode: Episode, podcast_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Smart discovery found {len(sources)} transcript sources for: {episode.title}")
    ...
    def _apply_known_patterns(self, episode: Episode, podcast_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for pattern in self.patterns[podcast_title]:
            try:
                ...
                candidate_urls = self._generate_urls_from_pattern(pattern, episode, podcast_config)
                ...
                for url in candidate_urls:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            # Extract content
                            content = self._extract_content(response.text, pattern.selector)
    ...
    def _generate_urls_from_pattern(self, pattern: TranscriptPattern, episode: Episode, 
                                   podcast_config: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            episode_data = {
                ...
                'published_year': episode.published_at.year if episode.published_at else '',
                'published_month': episode.published_at.month if episode.published_at else '',
                'published_day': episode.published_at.day if episode.published_at else '',
            }
    ...
    def _smart_discovery(self, episode: Episode, podcast_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for pattern in discovery_patterns[:10]:  # Limit attempts
            try:
                ...
                if response.status_code == 200:
                    ...
                    if content and len(content) > 500:
                        confidence = self._calculate_discovery_confidence(url, content, episode)
    ...
    def _try_pattern_variations(self, episode: Episode, podcast_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for pattern in all_patterns[:5]:
            try:
                urls = self._generate_urls_from_pattern(pattern, episode, podcast_config)
                ...
                for url in urls[:2]:  # Limit attempts per pattern
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if content and len(content) > 500:
                                confidence = self._calculate_discovery_confidence(url, content, episode) * 0.8  # Lower confidence for cross-podcast patterns
    ...
    def _extract_content(self, html: str, selector: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            # Generic extraction
            selectors = [
                '.transcript', '.transcription', '.episode-transcript',
                '.post-content', '.entry-content', '.content',
                'main', 'article', '[data-transcript]'
            ]
    ...
    def _calculate_discovery_confidence(self, url: str, content: str, episode: Episode) -> float:
    ...
    def _learn_pattern(self, source: Dict[str, Any], episode: Episode, podcast_config: Dict[str, Any]):
    ...
    def _generalize_url(self, url: str, episode: Episode, podcast_config: Dict[str, Any]) -> Optional[str]:
