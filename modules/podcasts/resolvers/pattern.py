...
class PatternResolver:
        ...
        self.site_patterns = {
            'slate.com': {
                'episode_to_transcript': [
                    ...
                    lambda url: f"{url}/transcript" if not url.endswith('/') else f"{url}transcript",
                ],
                'confidence_base': 0.6
            },
        ...
        try:
            ...
            if domain_key in self.site_patterns:
                ...
                for i, pattern_func in enumerate(patterns):
                    try:
                        ...
                        logger.debug(f"Error applying site pattern {i} to {episode.url}: {e}")
            ...
            for i, pattern_func in enumerate(self.generic_patterns):
                try:
                    ...
                    logger.debug(f"Error applying generic pattern {i} to {episode.url}: {e}")
            ...
            logger.info(f"Pattern resolver generated {len(sources)} potential URLs for: {episode.title}")
    ...
    def _generate_rss_based_patterns(self, episode: Episode, 
    ...
    def validate_pattern_url(self, url: str) -> bool:
        ...
        try:
            ...
            if any(bad in url.lower() for bad in [
                'example.com', 'localhost', 'test.', '.test'
            ]):
                return False
