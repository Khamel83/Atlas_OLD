...
class RSSLinkResolver:
    ...
    def resolve(self, episode: Episode, podcast_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if description:
                description_links = self._extract_transcript_urls_from_text(description, episode.url)
            ...
            logger.info(f"RSS resolver found {len(sources)} transcript sources for: {episode.title}")
    ...
    def _calculate_confidence(self, url: str, episode: Episode) -> float:
        ...
        if any(trusted in domain for trusted in [
            'rev.com', 'otter.ai', 'transcript.com', 'transcripts.com'
        ]):
            confidence += 0.3
