...
class GoogleSearchResolver:
    ...
    def resolve(self, episode: Episode, podcast_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Google search found {len(sources)} potential transcript sources for: {episode.title}")
            
        except Exception as e:
            logger.error(f"Error in Google search resolver for episode {episode.title}: {e}")
    ...
    def _build_search_queries(self, episode: Episode, podcast_config: Dict[str, Any]) -> List[str]:
    ...
    def _clean_for_search(self, text: str) -> str:
        ...
        text = re.sub(r'\s+(podcast|show|radio|with\s+\w+)$', '', text, flags=re.IGNORECASE)
        ...
        try:
            ...
            for result_div in search_results[:10]:
                try:
                    ...
                    # Extract snippet
                    snippet_elem = result_div.find(['span', 'div'], 
                                                 {'class': re.compile(r'st|s3v9rd|VwiC3b')})
    ...
    def _is_relevant_result(self, url: str, title: str, snippet: str) -> bool:
        ...
        transcript_indicators = [
            'transcript', 'transcription', 'transcribed', 'full text',
            'episode notes', 'show notes', 'conversation', 'interview',
            'complete text', 'written version', 'text version'
        ]
        
        has_transcript_indicator = any(indicator in text_to_check 
                                     for indicator in transcript_indicators)
        
        # Good domains for transcripts
        good_domains = [
            'rev.com', 'otter.ai', 'temi.com', 'scribie.com',
            'reddit.com', 'medium.com', 'substack.com',
            'transcript.com', 'podscripts.co'
        ]
    ...
    def _process_search_result(self, result: Dict[str, Any], episode: Episode, 
            ...
            if confidence > 0.2:  # Only process promising results
                ...
                try:
                    ...
                    if transcript_content:
                        final_confidence = confidence + self._calculate_content_confidence(
    ...
    def _extract_transcript_content(self, html: str) -> Optional[str]:
        ...
        try:
            ...
            selectors = [
                ...
                '.show-notes', '.episode-notes', '.content',
    ...
    def _calculate_search_confidence(self, result: Dict[str, Any], episode: Episode, 
                                   query: str) -> float:
