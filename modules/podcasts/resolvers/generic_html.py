...
class GenericHTMLResolver:
    ...
    def resolve(self, episode: Episode, podcast_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"HTML resolver found {len(sources)} transcript sources for: {episode.title}")
    ...
    def _extract_from_url(self, url: str, episode: Episode, 
        ...
        try:
            ...
            if transcript_selector:
                transcript_content = self._extract_with_selector(html, transcript_selector)
                ...
                if confidence > 0.3:  # Only include if reasonably confident
                    sources.append({
                        ...
                        'metadata': {
                            ...
                            'extraction_method': 'css_selector' if transcript_selector else 'generic',
    ...
    def _looks_like_transcript(self, text: str) -> bool:
        ...
        transcript_keywords = [
            'transcript', 'interview', 'conversation', 'discussion',
            'host', 'guest', 'speaker', 'moderator'
        ]
        
        keyword_count = sum(1 for keyword in transcript_keywords 
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                if any(word in text for word in [
                    'transcript', 'transcription', 'full text', 'read more',
                    'view transcript', 'episode transcript'
                ]):
                    full_url = urljoin(base_url, href)
                    links.append(full_url)
                    
                # Check URL path
                elif any(word in href.lower() for word in [
    ...
    def _calculate_confidence(self, url: str, content: str, episode: Episode) -> float:
        ...
        if episode.title:
            ...
            title_word_matches = sum(1 for word in title_words 
