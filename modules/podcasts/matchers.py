...
class URLMatcher:
    ...
    def is_transcript_url(self, url: str, podcast_slug: str = "") -> float:
        ...
        if any(domain.endswith(d) for d in [
            'transcript.com', 'transcripts.com', 'rev.com'
        ]):
            confidence += 0.5
            
        return min(confidence, 1.0)
    
    def extract_episode_links(self, html: str, base_url: str, 
                            episode_selector: str = "") -> List[str]:
            ...
            if episode_selector:
                ...
                for link in soup.find_all('a', href=True):
                    ...
                    if any(word in text for word in ['episode', 'ep ', 'listen']):
                        links.append(self._resolve_url(href, base_url))
                    elif any(word in href.lower() for word in ['episode', 'ep-', '/episodes/']):
                        links.append(self._resolve_url(href, base_url))
    ...
    def extract_transcript_content(self, html: str, 
                                 transcript_selector: str = "") -> Optional[str]:
...
class PodcastMatcher:
    ...
    def rank_transcript_candidates(self, urls: List[str], 
                                 podcast_slug: str) -> List[tuple]:
