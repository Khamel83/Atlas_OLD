...
class PodcastStore:
    ...
    def _init_database(self):
        ...
        with self._get_connection() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_episodes_status ON episodes (transcript_status)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_transcript_sources_episode_id ON transcript_sources (episode_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_discovery_runs_podcast_id ON discovery_runs (podcast_id)")
    ...
    def create_podcast(self, podcast: Podcast) -> int:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                podcast.name, podcast.slug, podcast.rss_url, podcast.site_url,
                podcast.resolver, podcast.episode_selector, podcast.transcript_selector,
        ...
        with self._get_connection() as conn:
            ...
            if row:
                return Podcast(
                    ...
                    rss_url=row['rss_url'], site_url=row['site_url'],
                    resolver=row['resolver'], episode_selector=row['episode_selector'],
    ...
    def get_podcast_by_slug(self, slug: str) -> Optional[Podcast]:
        """Get podcast by slug"""
        with self._get_connection() as conn:
            row = conn.execute("SELECT * FROM podcasts WHERE slug = ?", (slug,)).fetchone()
            if row:
                return Podcast(
                    id=row['id'], name=row['name'], slug=row['slug'],
                    rss_url=row['rss_url'], site_url=row['site_url'],
    ...
    def create_episode(self, episode: Episode) -> int:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                episode.podcast_id, episode.guid, episode.title, episode.url,
                episode.publish_date, episode.transcript_url, episode.transcript_status,
                episode.transcript_path, self._serialize_dict(episode.metadata)
            ))
            return cursor.lastrowid
        ...
        with self._get_connection() as conn:
            ...
            for row in rows:
                episodes.append(Episode(
                    id=row['id'], podcast_id=row['podcast_id'], guid=row['guid'],
                    title=row['title'], url=row['url'], publish_date=row['publish_date'],
                    transcript_url=row['transcript_url'], transcript_status=row['transcript_status'],
    ...
    def update_episode_transcript_status(self, episode_id: int, status: str, 
                                       transcript_path: Optional[str] = None):
    ...
    def create_discovery_run(self, run: DiscoveryRun) -> int:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                run.podcast_id, run.resolver, run.episodes_found, run.transcripts_found,
                run.errors, run.duration_seconds, run.status, run.started_at
            ))
            return cursor.lastrowid

    def complete_discovery_run(self, run_id: int, episodes_found: int, 
                             transcripts_found: int, errors: int, 
        ...
        with self._get_connection() as conn:
            conn.execute("""
                ...
                WHERE id = ?
            """, (episodes_found, transcripts_found, errors, duration_seconds, status, run_id))
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._get_connection() as conn:
            ...
            # Podcast count
            stats['total_podcasts'] = conn.execute("SELECT COUNT(*) FROM podcasts").fetchone()[0]
            ...
            """).fetchall()
            stats['episodes_by_status'] = {row['transcript_status']: row['count'] for row in episode_stats}
