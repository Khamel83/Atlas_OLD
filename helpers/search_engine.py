...
class AtlasSearchEngine:
    ...
    def __init__(self, config: Optional[Dict] = None, host: str = None, api_key: str = None):
    ...
    def setup_index(self, reset: bool = False) -> bool:
        ...
        try:
            ...
            # Create or get index
            try:
                self.index = self.client.create_index(self.index_name, {'primaryKey': 'uid'})
    ...
    def index_content(self, content_type: Optional[ContentType] = None, 
        ...
        try:
            ...
            for item in all_metadata:
                try:
                    ...
                    if search_doc:
                        ...
                        result['content_types_processed'].add(item.get('content_type', 'unknown'))
                    ...
                    result['errors'].append(f"Error preparing {item.get('uid', 'unknown')}: {str(e)}")
            ...
            logger.info(f"Indexing completed: {result['indexed_count']} documents in {result['processing_time']:.2f}s")
        ...
        try:
            ...
            for hit in results['hits']:
                ...
                for tag in hit.get('tags', []):
                    if (query.lower() in tag.lower() and 
                        tag.lower() not in [s.lower() for s in suggestions] and
    ...
    def get_facets(self, query: str = "", facet_attributes: Optional[List[str]] = None) -> Dict:
        """Get faceted search results for filtering."""
        facet_attributes = facet_attributes or ['content_type', 'tags', 'source', 'author']
...
def get_search_engine(config: Optional[Dict] = None) -> AtlasSearchEngine:
    ...
    if not MEILISEARCH_AVAILABLE:
        raise ImportError("Meilisearch not available. Install with: pip install meilisearch")
...
def search_content(query: str, content_type: Optional[str] = None, 
