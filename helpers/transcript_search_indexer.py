...
class TranscriptSearchIndexer:
    ...
    def _init_database(self):
        ...
        try:
            ...
            if schema_path.exists():
                ...
                self.logger.info(f"Initialized transcript search database: {self.db_path}")
            else:
    ...
    def index_transcript_content(self, content_uid: str, parsed_transcript: Dict[str, Any], 
        ...
        try:
            ...
            cursor.execute("DELETE FROM transcript_segments WHERE content_uid = ?", (content_uid,))
            
            # Index segments
            segments_indexed = self._index_segments(cursor, content_uid, parsed_transcript, metadata)
            
            # Build topic clusters
            self._update_topic_clusters(cursor, parsed_transcript.get('topics', []))
            
            # Update speaker statistics
            self._update_speaker_stats(cursor, parsed_transcript.get('speakers', []), 
                                     parsed_transcript.get('segments', []))
            
            conn.commit()
            conn.close()
            
            self.logger.info(f"Indexed {segments_indexed} transcript segments for {content_uid}")
            return True
    ...
    def _index_segments(self, cursor: sqlite3.Cursor, content_uid: str, 
                       parsed_transcript: Dict[str, Any], metadata: Dict[str, Any]) -> int:
        ...
        for segment in segments:
            try:
                ...
                topic_tags = self._extract_segment_topics(segment, parsed_transcript.get('topics', []))
                ...
                self.logger.error(f"Error indexing segment {segment.get('id', 'unknown')}: {e}")
                continue
        
        return indexed_count
    
    def _extract_segment_topics(self, segment: Dict[str, Any], topics: List[Dict[str, Any]]) -> List[str]:
    ...
    def _update_topic_clusters(self, cursor: sqlite3.Cursor, topics: List[Dict[str, Any]]):
        ...
        for topic in topics:
            ...
            try:
                ...
                cursor.execute("SELECT id, content_count FROM topic_clusters WHERE topic_name = ?", (topic_name,))
    ...
    def _update_speaker_stats(self, cursor: sqlite3.Cursor, speakers: List[str], segments: List[Dict[str, Any]]):
        """Update speaker statistics."""
        ...
        for speaker, stats in speaker_stats.items():
            try:
                ...
                cursor.execute("SELECT id, segment_count, total_word_count FROM transcript_speakers WHERE normalized_name = ?", (normalized_name,))
                result = cursor.fetchone()
                ...
                if result:
                    ...
                    """, (current_segments + stats['segment_count'], 
                          current_words + stats['word_count'], speaker_id))
                else:
                    # Create new speaker
                    cursor.execute("""
                    ...
                    """, (speaker, normalized_name, stats['segment_count'], stats['word_count']))
    ...
    def search_transcripts(self, query: str, speaker: str = None, topic: str = None, 
                          segment_type: str = None, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(f"Search returned {len(search_results)} results for query: {query}")
    ...
    def find_related_segments(self, segment_id: int, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cursor.execute("SELECT topic_tags FROM transcript_segments WHERE id = ?", (segment_id,))
            result = cursor.fetchone()
    ...
    def get_search_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(DISTINCT speaker) FROM transcript_segments WHERE speaker IS NOT NULL")
            stats['unique_speakers'] = cursor.fetchone()[0]
            
            cursor.execute("SELECT COUNT(DISTINCT content_uid) FROM transcript_segments")
            ...
            """)
            stats['top_topics'] = [
                {'topic': row[0], 'content_count': row[1]} 
                for row in cursor.fetchall()
            ]
