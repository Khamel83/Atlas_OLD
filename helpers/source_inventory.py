...
class SourceInventoryDiscovery:
    ...
    def discover_podcast_work(self) -> List[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute("""
                    SELECT id, title, audio_url, podcast_name, rss_url, description
                ...
                for row in cursor.fetchall():
                    episode_id, title, audio_url, podcast_name, rss_url, description = row
                    unprocessed_episodes.append({
                        'url': audio_url,
                        'title': title or f"Episode from {podcast_name}",
                        'source': f'podcast_{podcast_name}',
