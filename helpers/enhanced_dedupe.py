...
class EnhancedDeduplicator:
    ...
    def find_duplicates(self, content_type: ContentType, candidate_metadata: Dict) -> List[SimilarityMatch]:
    ...
    def _compare_content(self, candidate: Dict, existing: Dict) -> Optional[SimilarityMatch]:
        ...
        if candidate_title and existing_title:
            similarities['title_exact'] = 1.0 if candidate_title == existing_title else 0.0
    ...
    def _calculate_confidence(self, similarities: Dict[str, float], best_match: Dict) -> float:
    ...
    def is_content_duplicate(self, content_type: ContentType, metadata: Dict, 
                           similarity_threshold: float = 0.8) -> Tuple[bool, Optional[SimilarityMatch]]:
        ...
        is_duplicate = (
            best_match.similarity_score >= similarity_threshold and 
...
def create_enhanced_deduplicator(metadata_manager: MetadataManager, 
                               path_manager: PathManager) -> EnhancedDeduplicator:
...
def enhanced_similarity_check(content1: Dict, content2: Dict) -> float:
    ...
    title_sim = JaccardSimilarity.text_similarity(
        content1.get('title', ''), content2.get('title', ''), 'words'
    ) if content1.get('title') and content2.get('title') else 0.0
    
    content_sim = JaccardSimilarity.text_similarity(
        content1.get('content', ''), content2.get('content', ''), 'words'
    ) if content1.get('content') and content2.get('content') else 0.0
    
    # Weight title more heavily
    return 0.6 * title_sim + 0.4 * content_sim


def detect_near_duplicates(items: List[Dict], threshold: float = 0.8) -> List[Tuple[str, str, float]]:
