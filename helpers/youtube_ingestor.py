...
class YouTubeIngestor(BaseIngestor):
    ...
    def ingest_single_video(self, url: str) -> bool:
        ...
        try:
            ...
            comprehensive_metadata = self._extract_all_youtube_metadata(
                yt, video_id, full_url
            )
            ...
            self.save_raw_data(
                comprehensive_metadata["raw_youtube_data"], meta, "youtube_api"
            )
    ...
    def _extract_all_youtube_metadata(
        self, yt: YouTube, video_id: str, full_url: str
    ) -> Dict[str, Any]:
        """
            ...
            if hasattr(yt, "streams"):
                ...
                for stream in yt.streams:
                    stream_info = {
                        ...
                        "includes_audio_track": getattr(
                            stream, "includes_audio_track", None
                        ),
                        "includes_video_track": getattr(
                            stream, "includes_video_track", None
                        ),
        ...
        youtube_data = {
            ...
            "publish_date": str(getattr(yt, "publish_date", None)),
            "upload_date": str(
                getattr(yt, "publish_date", None)
            ),  # Same as publish_date for YouTube
            ...
            "tags": getattr(
                yt, "keywords", []
            ),  # YouTube keywords are effectively tags
            ...
            "estimated_reading_time": self._estimate_transcript_reading_time(
                getattr(yt, "length", 0)
            ),
            # Technical information
            "stream_count": len(getattr(yt, "streams", [])),
            "has_video": any(
                getattr(s, "includes_video_track", False)
                for s in getattr(yt, "streams", [])
            ),
            "has_audio": any(
                getattr(s, "includes_audio_track", False)
                for s in getattr(yt, "streams", [])
            ),
