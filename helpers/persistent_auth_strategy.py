...
class PersistentAuthStrategy(ArticleFetchStrategy):
    ...
    def _create_authenticated_context(self, site: str, log_path: str = "") -> Optional[BrowserContext]:
        ...
        try:
            ...
            if session_data:
                ...
                if test_result:
                    ...
                    log_info(log_path, f"Existing {site} session invalid, logging in fresh")
                    page.close()
                    context.close()
            ...
            log_error(log_path, f"Failed to create authenticated context for {site}: {e}")
            return None
    ...
    def _login_nytimes(self, page: Page, username: str, password: str, log_path: str = "") -> bool:
        """Login to NYTimes."""
        try:
            ...
            for login_url in login_urls:
                try:
                    ...
                    login_selectors = [
                        ('input[data-testid="email"]', 'input[data-testid="password"]', 'button[data-testid="login-button"]'),
                        ...
                        try:
                            if page.locator(email_sel).is_visible():
                                ...
                                if "subscriber" in content and "dashboard" in content:
                                    log_info(log_path, "NYTimes login successful (content check)")
                                    return True
    ...
    def _login_wsj(self, page: Page, username: str, password: str, log_path: str = "") -> bool:
    ...
    def fetch(self, url: str, log_path: str = "") -> FetchResult:
        ...
        if not site_type:
            return FetchResult(success=False, error="Not a supported paywall site", method="persistent_auth")
        
        credentials = self._get_credentials(site_type)
        if not credentials:
            return FetchResult(success=False, error=f"{site_type.upper()} credentials not configured", method="persistent_auth")
        
        try:
            log_info(log_path, f"Attempting persistent authenticated fetch for {site_type.upper()}: {url}")
            ...
            if not context:
                return FetchResult(success=False, error=f"Failed to authenticate with {site_type}", method="persistent_auth")
            ...
            if len(content) > 1000:  # Basic quality check
                ...
                return FetchResult(success=False, error="Content too short", method="persistent_auth")
                
        except Exception as e:
            log_error(log_path, f"{site_type.upper()} persistent auth failed for {url}: {e}")
