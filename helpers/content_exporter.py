...
class ContentExporter:
    ...
    def __init__(self, db_path: str = None, templates_dir: str = None):
        ...
        if db_path is None:
            db_path = str(Path(__file__).parent.parent / "data" / "podcasts" / "atlas_podcasts.db")
        self.db_path = db_path
        self.templates_dir = templates_dir or Path(__file__).parent.parent / "exports" / "templates"
    ...
    def batch_export(self, 
        ...
        for i, config in enumerate(export_configs):
            if progress_callback:
                progress_callback(i, total, f"Exporting {config.get('format_type', 'unknown')}")
    ...
    def _get_filtered_content(self, filters: Dict) -> List[Dict]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            where_conditions = ["e.transcript_path IS NOT NULL"]  # Only episodes with transcripts
            params = []
            ...
            if filters.get("content_type"):
                ...
                if filters["content_type"] != "podcast" and filters["content_type"] != "transcript":
    ...
    def _format_content(self, content_data: List[Dict], format_type: str, template: str = None) -> Any:
    ...
    def _format_markdown(self, content_data: List[Dict], template: str = None) -> List[Dict]:
    ...
    def _format_obsidian(self, content_data: List[Dict]) -> List[Dict]:
        ...
        for content_item, segments in content_groups.items():
            ...
            frontmatter = {
                ...
                "speakers": list(set(s["speaker"] for s in segments if s.get("speaker"))),
                "topics": list(set(s["topic_cluster"] for s in segments if s.get("topic_cluster"))),
                "tags": [f"#{content_item.get('content_type', 'content')}", 
            ...
            if segments:
                ...
                for segment in segments:
                    if segment.get("speaker") and segment.get("transcript_segment"):
                        ...
                        content_lines.append(f"**{speaker_link}**: {segment['transcript_segment']}")
    ...
    def _format_notion(self, content_data: List[Dict]) -> Dict:
        ...
        for content_item, segments in content_groups.items():
            entry = {
                ...
                "Speakers": ", ".join(set(s["speaker"] for s in segments if s.get("speaker"))),
                "Topics": ", ".join(set(s["topic_cluster"] for s in segments if s.get("topic_cluster"))),
        ...
        for content_item, segments in content_groups.items():
                    ...
                    if len(segment_text) > 100 and "?" in segment_text:
                        ...
                        if len(parts) == 2:
                            ...
                            flashcards.append({
                                ...
                                "deck": podcast_title,
                                "tags": [content_item.get("content_type", "transcript"), 
                                        speaker.replace(" ", "_"),
                                        segment.get("topic_cluster", "general").replace(" ", "_")]
                            })
            ...
            if content_item.get("summary"):
                flashcards.append({
                    ...
                    "topic": "Summary",
                    "deck": podcast_title,
                    "tags": ["summary", content_item.get("content_type", "content")]
                })
    ...
    def _group_content_by_item(self, content_data: List[Dict]) -> Dict:
        ...
        for row in content_data:
            ...
            if content_id not in groups:
                ...
                groups[row] = []  # Empty segments list since we don't have parsed segments yet
        
        return groups
    
    def _write_export(self, formatted_content: Any, format_type: str, output_path: str) -> List[str]:
        ...
        if format_type in ["markdown", "obsidian"]:
            ...
            file_path = output_dir / f"atlas_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(formatted_content)
            created_files.append(str(file_path))
            
        elif format_type == "csv":
            file_path = output_dir / f"atlas_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(formatted_content)
            created_files.append(str(file_path))
            ...
            file_path = output_dir / f"notion_import_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            with open(file_path, "w", encoding="utf-8") as f:
            ...
            file_path = output_dir / f"anki_cards_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            
            if formatted_content:
                with open(file_path, "w", encoding="utf-8", newline="") as f:
                    fieldnames = ["front", "back", "speaker", "source", "topic", "deck", "tags"]
