...
from helpers.article_strategies import ArticleFetchStrategy, FetchResult, ContentAnalyzer
...
class FirecrawlStrategy(ArticleFetchStrategy):
    ...
    def _load_usage_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if data.get("month") != current_month:
                log_info("", f"Resetting Firecrawl usage counter for new month: {current_month}")
                data = {
                    "month": current_month,
                    "usage_count": 0,
    ...
    def _check_usage_limit(self) -> bool:
        ...
        if current_usage >= self.monthly_limit:
            log_info("", f"Firecrawl monthly limit reached: {current_usage}/{self.monthly_limit}")
            return False
        
        log_info("", f"Firecrawl usage: {current_usage}/{self.monthly_limit} this month")
        return True
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("success"):
                    ...
                    final_content = markdown_content if markdown_content else html_content
                    
                    if len(final_content) > 500:  # Quality check
                        title = metadata.get("title") or ContentAnalyzer.extract_title_from_html(html_content)
                        
                        # Check if content looks good
                        is_truncated = ContentAnalyzer.is_likely_paywall(final_content, log_path)
    ...
    def get_usage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (data.get("successful_requests", 0) / max(1, data.get("usage_count", 1))) * 100
        }