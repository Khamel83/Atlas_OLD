...
class AtlasDailyReporter:
    ...
    def _get_ingestion_summary(self, date: str) -> Dict[str, Any]:
        ...
        for category in ["articles", "podcasts", "youtube", "documents"]:
            ...
            if category_path.exists():
                ...
                today_files = [f for f in files if 
                             datetime.fromtimestamp(f.stat().st_mtime) >= cutoff and
                             datetime.fromtimestamp(f.stat().st_mtime) < next_day]
    ...
    def _get_next_actions(self) -> List[str]:
        ...
        try:
            ...
            if "atlas_background_service.py" not in result.stdout:
                actions.append("🔴 CRITICAL: Start background service with ./scripts/start_atlas_service.sh start")
        except Exception:
    ...
    def generate_push_notification(self, report: Dict[str, Any]) -> str:
        ...
        # Create summary text
        notification = f"{status} Atlas Daily: {summary['total_files_today']} items processed"
