...
class ConversationRanking:
    ...
    def rank_transcript_results(self, results: List[Dict[str, Any]], query_type: str = 'general') -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(f"Ranked {len(ranked_results)} results for query type: {query_type}")
    ...
    def _calculate_conversation_score(self, result: Dict[str, Any], query_type: str) -> float:
        ...
        try:
            ...
            type_priorities = self.segment_priorities.get(query_type, self.segment_priorities['general'])
            ...
            speaker_score = self._calculate_speaker_importance(result.get('speaker', ''))
            total_score += speaker_score * self.weights['speaker_importance']
            
            # Word count factor (longer segments often more informative)
            word_count_score = self._calculate_word_count_score(result.get('word_count', 0))
    ...
    def _calculate_content_quality(self, content: str) -> float:
        ...
        explanation_words = ['because', 'therefore', 'however', 'actually', 'essentially', 'specifically']
        if any(word in content_lower for word in explanation_words):
            score += 0.2
        
        # Technical depth indicators
        technical_words = ['algorithm', 'system', 'process', 'method', 'approach', 'technology']
        if any(word in content_lower for word in technical_words):
            score += 0.1
        
        # Insight indicators
        insight_words = ['interesting', 'important', 'key', 'crucial', 'significant', 'realize']
    ...
    def rank_by_conversation_flow(self, results: List[Dict[str, Any]], 
                                 context_segments: List[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    def _apply_conversation_flow_scoring(self, episode_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _identify_qa_pairs(self, segments: List[Dict[str, Any]]) -> List[int]:
        ...
        for i in range(len(segments) - 1):
            ...
            if (current.get('segment_type') == 'question' and 
                next_segment.get('segment_type') == 'answer'):
            ...
            if (current_speaker != next_speaker and 
                ('?' in current.get('content', '') or 
                 any(word in current.get('content', '').lower() for word in ['what', 'how', 'why']))):
                qa_indices.extend([i, i + 1])
        
        return list(set(qa_indices))  # Remove duplicates
    
    def boost_guest_insights(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
