...
class SimpleAuthStrategy(ArticleFetchStrategy):
    ...
    def _login_nytimes(self, session: requests.Session, username: str, password: str, log_path: str = "") -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                log_error(log_path, f"Failed to access NYTimes login page: {response.status_code}")
                return False
            ...
            response = session.post(form_action, data=login_data, timeout=15, allow_redirects=True)
            ...
            log_error(log_path, f"NYTimes login failed - Status: {response.status_code}, URL: {response.url}")
            return False
    ...
    def _login_wsj(self, session: requests.Session, username: str, password: str, log_path: str = "") -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                log_error(log_path, f"Failed to access WSJ login page: {response.status_code}")
                return False
            ...
            log_error(log_path, f"WSJ login failed - Status: {response.status_code}, URL: {response.url}")
            return False
    ...
    def _test_authentication(self, session: requests.Session, site: str, log_path: str = "") -> bool:
    ...
    def _get_authenticated_session(self, site: str, log_path: str = "") -> Optional[requests.Session]:
    ...
    def fetch(self, url: str, log_path: str = "") -> FetchResult:
        ...
        if not site_type:
            return FetchResult(success=False, error="Not a supported paywall site", method="simple_auth")
        
        credentials = self._get_credentials(site_type)
        if not credentials:
            return FetchResult(success=False, error=f"{site_type.upper()} credentials not configured", method="simple_auth")
        
        try:
            log_info(log_path, f"Attempting simple authenticated fetch for {site_type.upper()}: {url}")
            ...
            if not session:
                return FetchResult(success=False, error=f"Failed to authenticate with {site_type}", method="simple_auth")
            ...
            if len(content) > 1000:
                ...
                return FetchResult(success=False, error="Content too short", method="simple_auth")
                
        except Exception as e:
            log_error(log_path, f"{site_type.upper()} simple auth failed for {url}: {e}")
