...
from helpers.document_processor import AtlasDocumentProcessor, is_document_processing_available
...
class DocumentIngestor(BaseIngestor):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        print(f"[{self.module_name}] Initialized with {len(self.document_processor.get_supported_extensions())} supported formats")
    ...
    def _fetch_from_url(self, url: str, metadata: ContentMetadata) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if not ext or not self.document_processor.is_supported_format(f"dummy{ext}"):
    ...
    def _fetch_from_file(self, file_path: str, metadata: ContentMetadata) -> Tuple[bool, Optional[str]]:
        """Process local file."""
        try:
            # Handle file:// URLs
    ...
    def process_content(self, file_path: str, metadata: ContentMetadata) -> bool:
        ...
        try:
            ...
            if result['processing_status'] != 'completed':
                ...
                print(f"ERROR [{self.module_name}]: Document processing failed: {error_msg}")
                return False
            ...
            metadata.type_specific.update({
                'processing_metadata': {
                    ...
                    'document_format': doc_metadata.get('file_extension', 'unknown'),
                    'file_size': doc_metadata.get('file_size', 0),
                    'word_count': doc_metadata.get('word_count', 0),
                    'estimated_reading_time': doc_metadata.get('estimated_reading_time', 0),
                    'element_types': doc_metadata.get('element_types', {}),
                    'has_tables': doc_metadata.get('has_tables', False),
                    'structure_analysis': doc_metadata.get('structure_analysis', {}),
                },
            ...
            # Set title 
            metadata.title = metadata.title or self._extract_title_from_content(result['content'])
            ...
            if len(content) > 500:
                try:
                    ...
                    metadata.type_specific['summary'] = summarize_text(content[:4000])  # Limit for summarization
                except Exception as e:
                    print(f"ERROR [{self.module_name}]: Failed to generate summary: {str(e)}")
                    metadata.type_specific['summary'] = content[:500] + "..." if len(content) > 500 else content
            else:
            ...
            print(f"[{self.module_name}] Successfully processed document: {metadata.uid}")
            return True
            ...
            if is_temp_file and file_path and os.path.exists(file_path):
                try:
                    os.unlink(file_path)
                    print(f"[{self.module_name}] Cleaned up temporary file: {file_path}")
                except Exception as e:
                    print(f"ERROR [{self.module_name}]: Failed to clean up temp file {file_path}: {str(e)}")
    ...
    def _extract_title_from_content(self, content: str) -> str:
        ...
        for line in lines[:5]:  # Check first 5 lines
            ...
            if line and len(line) < 200:  # Reasonable title length
                ...
                if not any(skip in line.lower() for skip in ['table of contents', 'page', 'chapter']):
                    return line
    ...
    def ingest_content(self, source: str, **kwargs) -> IngestorResult:
        ...
        try:
            ...
            if self.should_skip(source):
                print(f"[{self.module_name}] Skipping already processed document: {source}")
            ...
            if not success:
                ...
                return IngestorResult(success=False, error=file_path_or_error, should_retry=True)
    ...
    def batch_ingest_directory(self, directory_path: str, recursive: bool = True) -> Dict[str, Any]:
        """
        ...
        """
        try:
            print(f"[{self.module_name}] Starting batch ingestion of directory: {directory_path}")
            ...
            print(f"[{self.module_name}] Found {results['total_found']} supported files")
            ...
            for file_path in files_to_process:
                try:
                    ...
                    print(f"ERROR [{self.module_name}]: Error processing {file_path}: {str(e)}")
            
            print(f"[{self.module_name}] Batch ingestion completed: {results['processed']} processed, "
