...
from helpers.enhanced_dedupe import (
...
class IntegratedDeduplicator:
    ...
    def is_content_duplicate(self, content_type: ContentType, metadata: Dict) -> Tuple[bool, Optional[SimilarityMatch]]:
    ...
    def check_all_duplicates(self, url: str, content_type: ContentType, 
        ...
        if metadata and self.content_check_enabled:
            ...
            if content_duplicate and similarity_match:
                ...
                if (similarity_match.match_type in ['title_exact', 'hash_match'] or 
                    similarity_match.confidence > 0.9):
                    result['recommendation'] = 'skip'
    ...
    def find_similar_content(self, content_type: ContentType, metadata: Dict, 
                           limit: int = 10) -> List[SimilarityMatch]:
        ...
        try:
            ...
            for content_type_str, items in by_type.items():
                ...
                for i, item in enumerate(items):
                    matches = self.enhanced_deduplicator.find_duplicates(content_type, item)
                ...
                stats['content_types'][content_type_str] = {
                    ...
                    'high_confidence_duplicates': len([d for d in duplicates if d.confidence > 0.9])
                }
                
                stats['potential_duplicates'] += len(duplicates)
                stats['high_confidence_duplicates'] += len([d for d in duplicates if d.confidence > 0.9])
    ...
    def cleanup_duplicates(self, dry_run: bool = True, confidence_threshold: float = 0.95) -> Dict:
                ...
                if not dry_run:
                    for match in type_duplicates:
                        try:
                            ...
                            result['errors'].append(f"Error removing {match.duplicate_uid}: {str(e)}")
...
def get_integrated_deduplicator(config: Optional[Dict] = None) -> IntegratedDeduplicator:
...
def is_duplicate_enhanced(url: str, content_type: ContentType, metadata: Optional[Dict] = None) -> bool:
...
def get_duplicate_info(url: str, content_type: ContentType, metadata: Optional[Dict] = None) -> Dict:
