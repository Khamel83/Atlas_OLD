...
Unified interface providing backward compatibility for system tests.
Re-exports AtlasServiceManager from atlas_service_manager for compatibility.
"""

# Import and re-export AtlasServiceManager for system test compatibility
from atlas_service_manager import AtlasServiceManager

# For backward compatibility, also support comprehensive service
import time
import sys
import subprocess
import json
...
class AtlasComprehensiveService:
    """The comprehensive Atlas background service that runs processing cycles"""
    ...
    def run_script(self, script_path, timeout=600, description=""):
        ...
        try:
            self.log(f"Starting: {description} - {script_path} (timeout: {timeout}s)")
            ...
            process = subprocess.Popen([
              ...
              cwd=Path(__file__).parent, bufsize=1, universal_newlines=True)
            ...
            while process.poll() is None:
                ...
                # Log progress every 30 seconds for long tasks  
                if elapsed > 30 and int(elapsed) % 30 == 0:
                    self.log(f"PROGRESS: {description} running for {elapsed:.0f}s...")
            ...
            if process.returncode == 0:
                ...
                self.log(f"ERROR: {description} failed after {elapsed:.1f}s - {stderr[:200]}")
                return False
    ...
    def comprehensive_processing_cycle(self):
        ...
        # Phase 1: Enhanced Transcript Discovery (INTEGRATED!)
        self.log("PHASE 1: ENHANCED TRANSCRIPT DISCOVERY")
        self.run_script("enhanced_transcript_discovery.py", 900, "Enhanced transcript discovery with YouTube + Networks")
