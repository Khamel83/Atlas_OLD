...
class AtlasMetricsExporter(BaseHTTPRequestHandler):
    def __init__(self, *args, **kwargs):
        ...
        self.db_path = "/home/ubuntu/dev/atlas/atlas.db"  # Default path, can be configured
    ...
    def get_article_metrics(self):
        ...
        try:
            if os.path.exists(self.db_path):
                ...
                cursor.execute("SELECT COUNT(*) FROM articles WHERE status = 'processed' AND processed_at > ?", 
                             (since.isoformat(),))
                recent_processed = cursor.fetchone()[0] or 0
                
                cursor.execute("SELECT COUNT(*) FROM articles WHERE status = 'failed' AND processed_at > ?", 
                             (since.isoformat(),))
    ...
    def get_podcast_metrics(self):
        ...
        try:
            if os.path.exists(self.db_path):
                ...
                cursor.execute("SELECT COUNT(*) FROM podcasts WHERE status = 'downloaded'")
            ...
            if df_result.returncode == 0:
                ...
                if len(lines) > 1:
                    ...
                    metrics['atlas_system_disk_usage_percent'] = {
                        ...
                        'values': [
                            {'labels': {}, 'value': disk_usage_percent}
                        ]
                    }
            ...
            if free_result.returncode == 0:
                ...
                if len(lines) > 1:
                    ...
                    if len(mem_info) >= 7:
                        ...
                        mem_usage_percent = int(((total_mem - avail_mem) / total_mem) * 100)
                        
                        metrics['atlas_system_memory_usage_percent'] = {
                            'type': 'gauge',
                            'help': 'System memory usage percentage',
                            'values': [
                                {'labels': {}, 'value': mem_usage_percent}
                            ]
                        }
    ...
    def check_background_service(self):
        ...
        try:
            ...
            result = subprocess.run(['pgrep', '-f', 'atlas_background'], 
