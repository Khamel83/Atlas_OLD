...
def install_grafana():
    ...
    run_command("wget -q -O - https://packages.grafana.com/gpg.key | sudo apt-key add -", "Adding Grafana GPG key")
    run_command('sudo add-apt-repository "deb https://packages.grafana.com/oss/deb stable main"', "Adding Grafana repository")
...
"""
    ...
    run_command("sudo cp /tmp/grafana.ini /etc/grafana/grafana.ini", "Copying Grafana configuration")
    run_command("sudo chown grafana:grafana /etc/grafana/grafana.ini", "Setting config ownership")
...
def create_dashboards():
    ...
    run_command("sudo mkdir -p /etc/grafana/provisioning/datasources", "Creating datasources directory")
    run_command("sudo mkdir -p /etc/grafana/provisioning/dashboards", "Creating dashboards directory")
...
"""
    ...
    run_command("sudo cp /tmp/prometheus_datasource.yml /etc/grafana/provisioning/datasources/prometheus.yml", "Installing datasource configuration")
...
"""
    ...
    run_command("sudo cp /tmp/dashboard_provider.yml /etc/grafana/provisioning/dashboards/atlas.yml", "Installing dashboard provider")
    
    # Create dashboards directory
    run_command("sudo mkdir -p /var/lib/grafana/dashboards", "Creating dashboards directory")
    run_command("sudo chown -R grafana:grafana /var/lib/grafana/dashboards", "Setting dashboard directory ownership")
            ...
            "rows": [
                {
                    "panels": [
                        ...
                        },
                        {
                            ...
                            "targets": [
                                {
                                    "expr": "atlas_processing_queue_length"
                                }
                            ],
                            "format": "none"
                        }
    ...
    run_command("sudo cp /tmp/atlas_overview.json /var/lib/grafana/dashboards/atlas_overview.json", "Installing Atlas overview dashboard")
    ...
    run_command("sudo cp /tmp/system_health.json /var/lib/grafana/dashboards/system_health.json", "Installing system health dashboard")
...
def start_services():
    ...
    run_command("sudo systemctl status grafana-server --no-pager -l || true", "Grafana status")
