...
def install_prometheus():
    ...
    run_command(f"wget {prometheus_url} -O /tmp/prometheus.tar.gz", "Downloading Prometheus")
    run_command("tar xvfz /tmp/prometheus.tar.gz -C /tmp", "Extracting Prometheus")
    run_command("sudo cp /tmp/prometheus-2.45.0.linux-amd64/prometheus /usr/local/bin/", "Installing Prometheus binary")
    run_command("sudo cp /tmp/prometheus-2.45.0.linux-amd64/promtool /usr/local/bin/", "Installing promtool")
    
    # Create prometheus user
    run_command("sudo useradd --no-create-home --shell /bin/false prometheus || true", "Creating prometheus user")
    ...
    # Set permissions
    run_command("sudo chown prometheus:prometheus /etc/prometheus", "Setting config permissions")
    run_command("sudo chown prometheus:prometheus /var/lib/prometheus", "Setting data permissions")
...
"""
    ...
    run_command("sudo cp /tmp/prometheus.yml /etc/prometheus/prometheus.yml", "Copying configuration")
    run_command("sudo chown prometheus:prometheus /etc/prometheus/prometheus.yml", "Setting config ownership")
...
"""
    ...
    run_command("sudo cp /tmp/prometheus.service /etc/systemd/system/prometheus.service", "Installing systemd service")
...
def install_node_exporter():
    ...
    run_command(f"wget {node_exporter_url} -O /tmp/node_exporter.tar.gz", "Downloading Node Exporter")
    run_command("tar xvfz /tmp/node_exporter.tar.gz -C /tmp", "Extracting Node Exporter")
    run_command("sudo cp /tmp/node_exporter-1.6.1.linux-amd64/node_exporter /usr/local/bin/", "Installing Node Exporter binary")
    
    # Create node_exporter user
    run_command("sudo useradd --no-create-home --shell /bin/false node_exporter || true", "Creating node_exporter user")
...
"""
    ...
    run_command("sudo cp /tmp/node_exporter.service /etc/systemd/system/node_exporter.service", "Installing Node Exporter service")
    run_command("sudo systemctl daemon-reload", "Reloading systemd")
...
"""
    ...
    run_command("sudo cp /tmp/atlas_metrics_exporter.py " + exporter_path, "Installing Atlas metrics exporter")
...
"""
    ...
    run_command("sudo cp /tmp/atlas_metrics_exporter.service /etc/systemd/system/atlas_metrics_exporter.service", "Installing exporter service")
    run_command("sudo systemctl daemon-reload", "Reloading systemd")
    run_command("sudo systemctl enable atlas_metrics_exporter", "Enabling exporter service")
...
def start_services():
    ...
    run_command("sudo systemctl start atlas_metrics_exporter", "Starting Atlas Metrics Exporter")
    run_command("sudo systemctl start prometheus", "Starting Prometheus")
    
    # Check service status
    print("\nService Status:")
    run_command("sudo systemctl status node_exporter --no-pager -l || true", "Node Exporter status")
    run_command("sudo systemctl status atlas_metrics_exporter --no-pager -l || true", "Atlas Metrics Exporter status")
    run_command("sudo systemctl status prometheus --no-pager -l || true", "Prometheus status")
