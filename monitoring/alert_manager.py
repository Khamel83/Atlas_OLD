...
class AlertManager:
    ...
    def send_email(self, subject, body):
        ...
        try:
            ...
            with smtplib.SMTP(
                self.smtp_config["smtp_server"], self.smtp_config["port"]
            ) as server:
...
"""
    ...
    def run_cycle(self):
        ...
        now = datetime.now()
        if (
            now.weekday() == self.config["schedule"]["weekly_summary_day"]
            and now.hour == self.config["schedule"]["weekly_summary_hour"]
            and now.minute < 5
        ):  # Send within first 5 minutes of scheduled hour
