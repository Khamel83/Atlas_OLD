...
class CaptureAPI:
    ...
    def __init__(self, config=None):
        ...
        self.db_path = str(Path(__file__).parent.parent / "data" / "podcasts" / "atlas_podcasts.db")
    ...
    def _queue_for_processing(self, capture_id, content_type, content, metadata, source_device):
...
def capture_endpoint():
    ...
    try:
        ...
        if not data:
            return jsonify({
                "success": False,
                "error": "Invalid JSON or empty request"
            }), 400
...
def recent_captures_endpoint():
    ...
    try:
        ...
        with sqlite3.connect(capture_api.db_path) as conn:
            ...
            return jsonify({
                "recent_captures": [dict(row) for row in results]
            })
            
    except Exception as e:
        return jsonify({
            "error": f"Recent captures error: {str(e)}"
        }), 500
