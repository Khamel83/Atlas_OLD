...
class TranscriptSearchAPI:
    ...
    def init_app(self, app: Flask):
        ...
        app.add_url_rule('/api/transcript/search', 'transcript_search', 
                        self.search_transcripts, methods=['GET', 'POST'])
        app.add_url_rule('/api/transcript/speaker/<speaker_name>/topics', 'speaker_topics',
                        self.get_speaker_topics, methods=['GET'])
        app.add_url_rule('/api/transcript/segment/<int:segment_id>/related', 'related_segments',
                        self.get_related_segments, methods=['GET'])
        app.add_url_rule('/api/transcript/speakers', 'list_speakers',
                        self.list_speakers, methods=['GET'])
        app.add_url_rule('/api/transcript/topics', 'list_topics',
                        self.list_topics, methods=['GET'])
        app.add_url_rule('/api/transcript/stats', 'search_stats',
                        self.get_search_stats, methods=['GET'])
    ...
    def search_transcripts(self):
        ...
        try:
            ...
            if query:
                results = self.ranker.rank_transcript_results(results, self._get_query_type(query))
    ...
    def get_speaker_topics(self, speaker_name: str):
        ...
        try:
            ...
            return jsonify({
                'error': 'Error retrieving speaker topics',
                'status': 'error'
            }), 500
    ...
    def get_related_segments(self, segment_id: int):
        ...
        try:
            ...
            return jsonify({
                'error': 'Error finding related segments',
                'status': 'error'
            }), 500
    ...
    def list_speakers(self):
        ...
        try:
            ...
            return jsonify({
                'error': 'Error retrieving speakers',
                'status': 'error'
            }), 500
    ...
    def get_search_stats(self):
        ...
        try:
            ...
            return jsonify({
                'error': 'Error retrieving search statistics',
                'status': 'error'
            }), 500
    ...
    def _get_query_type(self, query: str) -> str:
        ...
        # Question queries
        question_indicators = ['what', 'how', 'why', 'when', 'where', 'who', 'can', 'do', 'would', 'should']
        if any(word in query_lower for word in question_indicators) or '?' in query:
            return 'question'
        
        # Topic queries
        if any(word in query_lower for word in ['about', 'discuss', 'topic', 'regarding']):
            return 'topic'
        
        # Default
        return 'general'
    
    def _format_standard_results(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _format_detailed_results(self, results: List[Dict[str, Any]], query: str) -> List[Dict[str, Any]]:
        """Format search results with additional context and conversation flow."""
        formatted = []
        
        for result in results:
            # Get conversation context
            context = self._get_conversation_context(result.get('id'), result.get('content_uid'))
    ...
    def _get_conversation_context(self, segment_id: int, content_uid: str, context_lines: int = 2) -> Dict[str, Any]:
    ...
    def _calculate_relevance_score(self, result: Dict[str, Any], query: str) -> float:
        ...
        try:
            ...
            if result.get('segment_type') == 'answer' and any(q in query_lower for q in ['what', 'how', 'why']):
                score += 0.5
